...
class Payment(models.Model):
    ...
    def __str__(self):
        return f"Pagamento {self.id[:8]} - {self.user.email} - R$ {self.amount}"
...
class PaymentTransaction(models.Model):
    ...
    def __str__(self):
        return f"Transação {self.transaction_type} - R$ {self.amount}"
...
class Coupon(models.Model):
    ...
    def __str__(self):
        return f"Cupom {self.code} - {self.discount_value}{'%' if self.discount_type == 'percentage' else ' BRL'}"
...
class CouponUsage(models.Model):
    ...
    def __str__(self):
        return f"Uso de {self.coupon.code} por {self.user.email}" 